...
import dataio, training, loss_functions, modules_no_uncertainty
...
def main(args):
    ...
    for scan_id in scan_ids:
        wandb.init(
            name=f"no_uncertainty_{args.data.name}_{scan_id}_{args.run}",
        ...
        net = partial(modules_no_uncertainty.INR, args.uncertainty)
        ...
        for BASELEARN_ID in range(0, args.uncertainty.num_baselearners):
            ...
            bslrs.append(model)
            del model
        model = modules_no_uncertainty.Ensemble(bslrs)
        ...
        with torch.no_grad():
            ...
            for key, val in test_loss_dct.items():
                val_numpy = val.cpu().numpy() if torch.is_tensor(val) else val
                np.savetxt(
                    os.path.join(results_dir, f"{key}_{scan_id}.txt"), np.array([val_numpy])
                )
    ...
    for key, val in avg_scan_uncertainty.items():
        ...
        val_numpy = avg_scan_uncertainty[key].cpu().numpy() if torch.is_tensor(avg_scan_uncertainty[key]) else avg_scan_uncertainty[key]
        np.savetxt(
