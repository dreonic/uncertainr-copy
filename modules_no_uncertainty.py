...
class FCBlock(nn.Module):
    ...
    def __init__(
        ...
        # dictionary of non-linearities and their corresponding weight initialization schemes.
        nls_and_inits = {
            "sine": (
                Sine(omega_0),
        ...
        self._net = [] # List of network layers
    ...
    def sample_predict(self, coords, Nsamples):
        # For deterministic model, just return the same prediction Nsamples times
        output = self.forward(coords, sample=False)
...
class INR(nn.Module):
    """Base INR model class without uncertainty quantification"""
